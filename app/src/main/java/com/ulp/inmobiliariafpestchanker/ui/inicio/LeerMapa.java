...
public class LeerMapa implements OnMapReadyCallback {
    ...
    public void onMapReady(@NonNull GoogleMap googleMap) {
        ...
        marcadorSANLUIS.setTitle("Inmobiliria Pestchanker");
        ...
        fl.getLastLocation().addOnSuccessListener(context.getMainExecutor(), new OnSuccessListener<Location>() {
            ...
            public void onSuccess(Location location) {
                if (location != null) {
                    ...
                    editor.putFloat("coordenadax",lat);//("coordenadax",coor.x);
                    editor.putFloat("coordenaday",longi);
                    editor.commit();
                    Log.d("enviada",lat +"   "+longi);
                    ...
                    SharedPreferences.Editor editor=sp.edit();
                    Float lat = -33.29453F;
                    Float longi = -66.33187F;

                    editor.putFloat("coordenadax",lat);//("coordenadax",coor.x);
                    editor.putFloat("coordenaday",longi);
                    editor.commit();
                    Log.d("enviada no gps",lat +"   "+longi);
